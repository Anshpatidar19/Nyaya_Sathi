...
async function handle(res) {
  if (!res.ok) {
    ...
    // 422 is a gate refusing the input: the request was well formed, the
    // content just isn't something this platform acts on. Sending the same
    // thing again can only be refused again, so callers treat it as a reply
    // to show rather than a failure to retry.
